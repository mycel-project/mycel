...
class TestNodeRepositoryBasic:
    ...
    def test_update(self, node_repo, create_user, create_col):
        ...
        node = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "01")
        node.position = "02"
        node.due = 9999
...
class TestNodeRepositoryQueries:
    def test_get_by_collection(self, node_repo, create_user, create_col):
        ...
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "01")
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "02")
    ...
    def test_get_by_type(self, node_repo, create_user, create_col):
        ...
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "01")
        node_repo.create(col.id, NodeContent(), NodeType.FRAGMENT, "02")
        
        spores = node_repo.get_by_type(col.id, NodeType.SPORE.value)
        assert len(spores) == 1
        assert spores[0].type == NodeType.SPORE

    def test_get_due(self, node_repo, create_user, create_col):
        user, _ = create_user()
        col = create_col(user_id=user.id)
        
        now = int(time.time() * 1000)
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "01", due=now - 1000)
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "02", due=now + 10000)

        due_nodes = node_repo.get_due(col.id, now_ms=now)
    ...
    def test_hierarchy(self, node_repo, create_user, create_col):
        ...
        parent = node_repo.create(col.id, NodeContent(), NodeType.FRAGMENT, "01")
        child1 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "02", parent_id=parent.id)
        child2 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "03", parent_id=child1.id)
...
class TestNodeRepositoryPositions:
    def test_position_updates(self, node_repo, create_user, create_col):
        user, _ = create_user()
        col = create_col(user_id=user.id)
    ...
    def test_position_queries(self, node_repo, create_user, create_col):
        user, _ = create_user()
        col = create_col(user_id=user.id)
        
        n1 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "A")
        n2 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "B")
        n3 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "C")

        assert node_repo.count_by_collection(col.id) == 3
        assert node_repo.count_before_position(col.id, "B") == 1
        
        assert node_repo.get_position_at_offset(col.id, 1) == "B"
        
        assert node_repo.get_tail_key(col.id) == "C"

        assert node_repo.get_predecessor_position(col.id, "B", n2.id) == "A"
        assert node_repo.get_successor_position(col.id, "B", n2.id) == "C"

    def test_get_all_positions(self, node_repo, create_user, create_col):
        user, _ = create_user()
        col = create_col(user_id=user.id)
        
        n1 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "X")
        n2 = node_repo.create(col.id, NodeContent(), NodeType.SPORE, "Y")
        
        positions = node_repo.get_all_positions(col.id)
        assert len(positions) == 2
        assert positions[0] == (n1.id, "X")
        assert positions[1] == (n2.id, "Y")

class TestDueCountByTypeAndDay:
    def test_utc_basic_bucketing(self, node_repo, create_user, create_col):
        ...
        node_repo.create(col.id, NodeContent(), node_type, "0", due=due_a)
        node_repo.create(col.id, NodeContent(), node_type, "0", due=due_b)
    ...
    def test_tz_offset_shifts_bucket(self, node_repo, create_user, create_col):
        """
        ...
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=due_ms)
    ...
    def test_negative_tz_offset(self, node_repo, create_user, create_col):
        """
    ...
    def test_large_positive_offset(self, node_repo, create_user, create_col):
    ...
    def test_groups_by_type(self, node_repo, create_user, create_col):
        """Spores and fragments on the same day are returned as separate rows."""
        user, _ = create_user()
        col = create_col(user_id=user.id)
        
        day = "2026-05-19 10:00:00"
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=_due_ms(day))
        time.sleep(0.002)
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=_due_ms(day))
        time.sleep(0.002)
        node_repo.create(col.id, NodeContent(), NodeType.FRAGMENT, "0", due=_due_ms(day))
    ...
    def test_start_end_filter(self, node_repo, create_user, create_col):
        """Nodes outside the [start_ms, to_ms) range are excluded."""
        user, _ = create_user()
        col = create_col(user_id=user.id)
        
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=_due_ms("2026-05-01 12:00:00"))
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=_due_ms("2026-05-15 12:00:00"))
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=_due_ms("2026-05-31 12:00:00"))
    ...
    def test_nodes_split_across_days_by_timezone(self, node_repo, create_user, create_col):
        """
        ...
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=due_a)
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=due_b)
    ...
    def test_empty_default_collection(self, node_repo, create_user, create_col):
    ...
    def test_midnight_boundary(self, node_repo, create_user, create_col):
        ...
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=before)
        node_repo.create(col.id, NodeContent(), NodeType.SPORE, "0", due=after)
